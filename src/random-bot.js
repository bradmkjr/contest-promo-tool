...
function randomUser(){
	...
	const text = "SELECT user_name, user_id FROM account OFFSET floor(random()* (SELECT COUNT(*) FROM account) ) LIMIT 1;";
	const values = [];
	...
	client.query(text, values, (err, res) => {
	  if (err) {
		...
		callback(err, undefined );  
	  	...
	  	screen_name = res.rows[0].user_name;
	  	a = res.rows[0].user_id;
	  	getUser();	
	  }
	});
}
